...
bool Activity::RecordData(void){
    ...
    std::ofstream outGT(ground_truth_fi1e.c_str(), std::ios::app);
    if (outGT.is_open())
    {
        outGT << message_odom_.header.stamp << " " << t_gt.x() << " " << t_gt.y()
        << " " << t_gt .z() << " " << q_gt.x() << " " << q_gt.y() << " " << q_gt.z() << " "
        << q_gt.w() << std::endl;
        outGT.close();
    }
