...
//     result_delta_q = delta_q * Eigen::Quaterniond(1, w_mid(0) * T / 2, w_mid(1) * T / 2, w_mid(2) * T / 2);
//     Eigen::Vector3d un_acc_1 = delta_q * curr_a;
//     // 3. get a_mid:
//     a_mid = 0.5 * (un_acc_0 + un_acc_1);
//     // 4. update relative translation:
//    result_delta_p = delta_p + state.beta_ij_ * T + 0.5 * a_mid * T * T;
//     // 5. update relative velocity:
//     result_delta_v = delta_v + a_mid * T;

//     state.beta_ij_ = result_delta_v;
//     state.alpha_ij_ = result_delta_p;
//     Sophus::SO3d temp(result_delta_q);
//     state.theta_ij_ = temp;
...
//     Eigen::Vector3d w_x = 0.5 * (prev_w + curr_w);
//     Eigen::Vector3d a_0_x = prev_a;
//     Eigen::Vector3d a_1_x = curr_a;
...
//     //
//     // TODO: 2. set up F_
//     // MatrixXd F_ = MatrixXd::Zero(15, 15);
//     F_.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
//     F_.block<3, 3>(0, 3) = -0.25 * delta_q.toRotationMatrix() * R_a_0_x * T * T + 
//                             -0.25 * result_delta_q.toRotationMatrix() * R_a_1_x * (Eigen::Matrix3d::Identity() - R_w_x * T) * T * T;
//     F_.block<3, 3>(0, 6) = Eigen::MatrixXd::Identity(3,3) * T;
//     F_.block<3, 3>(0, 9) = -0.25 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * T * T;
//     F_.block<3, 3>(0, 12) = -0.25 * result_delta_q.toRotationMatrix() * R_a_1_x * T * T * -T;
//     F_.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity() - R_w_x * T;
//     F_.block<3, 3>(3, 12) = -1.0 * Eigen::MatrixXd::Identity(3,3) * T;
//     F_.block<3, 3>(6, 3) = -0.5 * delta_q.toRotationMatrix() * R_a_0_x * T + 
//                             -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x * (Eigen::Matrix3d::Identity() - R_w_x * T) * T;
//     F_.block<3, 3>(6, 6) = Eigen::Matrix3d::Identity();
//     F_.block<3, 3>(6, 9) = -0.5 * (delta_q.toRotationMatrix() + result_delta_q.toRotationMatrix()) * T;
//     F_.block<3, 3>(6, 12) = -0.5 * result_delta_q.toRotationMatrix() * R_a_1_x * T * -T;
//     F_.block<3, 3>(9, 9) = Eigen::Matrix3d::Identity();
//     F_.block<3, 3>(12, 12) = Eigen::Matrix3d::Identity();

//     //
//     // TODO: 3. set up B_:
//     // Eigen::MatrixXd G = Eigen::MatrixXd::Zero(15,18);
//     B_.block<3, 3>(0, 0) =  0.25 * delta_q.toRotationMatrix() * T * T;
//     B_.block<3, 3>(0, 3) =  0.25 * -result_delta_q.toRotationMatrix() * R_a_1_x  * T * T * 0.5 * T;
//     B_.block<3, 3>(0, 6) =  0.25 * result_delta_q.toRotationMatrix() * T * T;
...
//     B_.block<3, 3>(6, 0) =  0.5 * delta_q.toRotationMatrix() * T;
//     B_.block<3, 3>(6, 3) =  0.5 * -result_delta_q.toRotationMatrix() * R_a_1_x  * T * 0.5 * T;
//     B_.block<3, 3>(6, 6) =  0.5 * result_delta_q.toRotationMatrix() * T;
