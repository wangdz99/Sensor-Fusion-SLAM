...
bool GNSSINSSimFiltering::InitFusion(const YAML::Node& config_node) {
    ...
    LOG(ERROR) << "recevived Fusion method " << CONFIG.FUSION_METHOD;  // 有bug, 输出为空
    // if (CONFIG.FUSION_METHOD == "error_state_kalman_filter") {
        kalman_filter_ptr_ = std::make_shared<ErrorStateKalmanFilter>(config_node["error_state_kalman_filter"]);
    // } else {
