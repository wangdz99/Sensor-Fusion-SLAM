...
MatchingFlow::MatchingFlow(ros::NodeHandle& nh) {
    LOG(INFO) << "init matching flow" << std::endl;
    // subscriber:
...
bool MatchingFlow::Run() {
    // LOG(INFO) << "run matching flow" << std::endl;
    if (matching_ptr_->HasNewGlobalMap() && global_map_pub_ptr_->HasSubscribers()) {
        LOG(INFO) << "publish global map" << std::endl;
    ...
    if (matching_ptr_->HasNewLocalMap() && local_map_pub_ptr_->HasSubscribers()){
        LOG(INFO) << "publish local map" << std::endl;
    ...
    while(HasData()) {
        ...
        if (UpdateMatching()) {
            LOG(INFO) << "update matching" << std::endl;
            PublishData();
        }
    }
...
bool MatchingFlow::ReadData() {
    ...
    // LOG(INFO) << "reading data" << std::endl;
...
bool MatchingFlow::PublishData() {
    // LOG(INFO) << "publishing data" << std::endl;
